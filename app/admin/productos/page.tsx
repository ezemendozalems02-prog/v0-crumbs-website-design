...
export default function AdminProductosPage() {
  ...
  const loadData = useCallback(() => {
    startTransition(async () => {
      const filters: { categoria_id?: string; disponible?: boolean; search?: string } = {}
      if (filterCat) filters.categoria_id = filterCat
      ...
      <header className="bg-primary text-primary-foreground hidden lg:block sticky top-0 z-30 border-b border-primary-foreground/10">
        <div className="px-6 py-4 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-[family-name:var(--font-dm-serif)] text-xl">Productos</h1>
              <p className="text-xs text-primary-foreground/60">Gestioná el menú completo</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="bg-card rounded-2xl border border-border/40 p-4 flex flex-wrap gap-3 items-center">
          ...
          >
            ...
            {categorias.map((c) => (
              <option key={c.id} value={c.id}>{c.nombre} ({TIPO_LABEL[c.tipo_menu]})</option>
            ))}
          </select>
          <select
            value={filterDisp}
        ...
        {isLoading ? (
          ...
          <>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border/30 bg-background/50">
                    {["Producto", "Categoría", "Precio base", "Variantes", "Estado", ""].map((h) => (
                    ...
                    <tr key={p.id} className="hover:bg-background/60 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center justify-end gap-1.5">
                          <button onClick={() => { setEditingProducto(p); setModalOpen(true) }} className="p-1.5 rounded-lg text-foreground/40 hover:text-primary hover:bg-primary/10 transition-colors" title="Editar">
                            <Pencil className="w-4 h-4" />
                          </button>
                          <button onClick={() => handleDelete(p.id, p.nombre)} disabled={deletingId === p.id} className="p-1.5 rounded-lg text-foreground/40 hover:text-red-600 hover:bg-red-50 transition-colors disabled:opacity-40" title="Eliminar">
            ...
            <div className="lg:hidden space-y-3">
              {productos.map((p) => (
                <div key={p.id} className="bg-card rounded-2xl border border-border/40 p-4">
                  ...
                  <div className="flex items-center gap-2 mt-3 pt-3 border-t border-border/20">
                    <button onClick={() => handleToggle(p.id, p.disponible)} disabled={togglingId === p.id} className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-medium rounded-xl transition-colors ${p.disponible ? "bg-emerald-50 text-emerald-700" : "bg-background text-foreground/50"}`}>
                      {p.disponible ? <ToggleRight className="w-4 h-4" /> : <ToggleLeft className="w-4 h-4" />}
                      {p.disponible ? "Disponible" : "No disponible"}
                    </button>
                    <button onClick={() => { setEditingProducto(p); setModalOpen(true) }} className="p-2 rounded-xl bg-primary/10 text-primary"><Pencil className="w-4 h-4" /></button>
                    <button onClick={() => handleDelete(p.id, p.nombre)} className="p-2 rounded-xl bg-red-50 text-red-600"><Trash2 className="w-4 h-4" /></button>
                  </div>
                </div>
              ))}
            </div>
