...
async function seedDelivery() {
  console.log('[v0] Starting delivery menu seed...');
  ...
  try {
    ...
    console.log('[v0] Upserting categories...');
    const { error: catError } = await supabase
      .from('categorias')
      .upsert(categoriesData);

    if (catError) {
      console.error('[v0] Error upserting categories:', catError);
      throw catError;
    }

    console.log('[v0] Categories upserted successfully');

    // Obtener IDs de categorías
    const { data: allCategories, error: getCatError } = await supabase
      .from('categorias')
      .select('id, nombre');

    if (getCatError) {
      console.error('[v0] Error fetching categories:', getCatError);
      throw getCatError;
    }
    ...
    console.log('[v0] Category IDs:', categoryMap);
    ...
    if (prodError) {
      console.error('[v0] Error inserting products:', prodError);
      throw prodError;
    }

    console.log('[v0] Successfully seeded delivery menu!');
    console.log(`[v0] - Categories: ${categoriesData.length}`);
    console.log(`[v0] - Products: ${productsData.length}`);
  } catch (error) {
